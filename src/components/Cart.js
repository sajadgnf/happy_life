...
const Cart = () => {
  ...
  return checkout || itemsCounter < 1 ? (
    <div className={classes.complete}>
      <img
        ...
        alt="صبد خرید خالی"
      />
      <Typography variant="h6" marginBottom fontSize={16}>
        سبد خرید شما خالی است!
      </Typography>
      <Button variant="contained" sx={{ width: 130, p: 0 }}>
        <Link to="/" className={classes.completeBtn}>
          خرید
        </Link>
      </Button>
    </div>
          ...
          >
            ...
            >
              <Box display="flex" alignItems="center" marginBottom={1}>
                <IconButton
                  onClick={() => dispatch(removeItem(item, item.color))}
                >
                  <DeleteOutlineSharpIcon color="primary" />
                </IconButton>
                <Box className={classes.quantityButtonContainer}>
                  <Button
                    onClick={() => dispatch(increase(item, item.color))}
                  ...
                  <Button
                    onClick={() =>
                      item.quantity === 1
                        ? dispatch(removeItem(item, item.color))
                        : dispatch(decrease(item, item.color))
                    }
                    className={classes.quantityButton}
                  >
                    -
      ...
      >
        <CardContent sx={{ padding: 0 }}>
          ...
          <Box className={classes.paymentContainer} marginTop={2.5}>
            <Typography className={classes.payment} fontWeight={400}>
              جمع سبد خرید
            </Typography>
            <Typography className={classes.payment} fontWeight={400}>
              {Number(total).toLocaleString()} تومان
        ...
        <CardActions
          sx={{ display: "flex", justifyContent: "center", pt: 4, pb: 0 }}
        >
          <Button
            ...
            sx={{ py: 1.2 }}
            startIcon={
              <img src={wallet} alt="wallet" className={classes.paymentIcon} />
            }
          >
            پرداخت
          </Button>
