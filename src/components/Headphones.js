...
const Headphones = ({ show, productsState }) => {

    useTitle("خرید و قیمت هندزفری")
    ...
    const brands = productsState.products.headphones.map(item => item.brand).filter(unique)
    productsState.products.headphones.map(item => item.colors.map(item => colors.push(item)))
    ...
    useEffect(() => {
        ...
        if (categories.available && !categories.color) {
            if (categories.brand) {
                setHeadphones(headphones => headphones.filter(item => item.available))
            } else {
                setHeadphones(productsState.products.headphones.filter(item => item.available))
            }
        }

        if (!categories.available && !categories.color) {
            if (categories.brand) {
                setHeadphones(productsState.products.headphones.filter(item =>
                    item.brand === categories.brand
                ))
            } else {
                setHeadphones([...productsState.products.headphones])
            }
        }
        ...
        if (categories.color) {
            if (categories.brand && categories.available) {
                ...
                setHeadphones(productsState.products.headphones.filter(item => item.colors.find(color => color === categories.color)))
            }
        }

        if (categories.priceRange) {
            setHeadphones(headphones => headphones.filter(
                item => item.price <= categories.priceRange[1] &&
                    item.price >= categories.priceRange[0]
            ))
        }

        if (categories.price === "highestPrice") setHeadphones(headphones => headphones.sort((a, b) => b.price - a.price))
        if (categories.price === "lowestPrice") setHeadphones(headphones => headphones.sort((a, b) => a.price - b.price))
    ...
    useEffect(() => {
        if (categories.brand) {
            if (categories.available) {
                setHeadphones(productsState.products.headphones.filter(item => item.available && item.brand === categories.brand))
            }
                ...
                setHeadphones(productsState.products.headphones.filter(item => item.brand === categories.brand))
            }
        }
    }, [categories.brand])
