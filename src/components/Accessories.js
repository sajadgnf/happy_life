...
const Accessories = ({ show, productsState }) => {
  ...
  const [accessories, setAccessories] = useState([
    ...productsState.products.accessories,
  ]);
  ...
  const brands = productsState.products.accessories
    .map((item) => item.brand)
    .filter(unique);
  ...
  useEffect(() => {
    if (categories.available && !categories.color) {
      if (categories.brand) {
        setAccessories((accessories) =>
          accessories.filter((item) => item.available)
        );
      } else {
        setAccessories(
          productsState.products.accessories.filter((item) => item.available)
        );
      }
    }
    ...
    if (!categories.available && !categories.color) {
      if (categories.brand) {
        setAccessories(
          productsState.products.accessories.filter(
    ...
    if (categories.color) {
      if (categories.brand && categories.available) {
        ...
        setAccessories(
          productsState.products.accessories.filter((item) =>
            item.colors.find((color) => color === categories.color)
          )
        );
      }
